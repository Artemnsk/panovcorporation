...
public class SortedWorkList extends WorkList {
	 ...
	 * super(new(new))!!   
	 */
	private static Comparator<Box> _sorter = new AscendingLowBoundBoxComparator();

	public SortedWorkList() {
		super(new TreeSet<Box>(_sorter), null);
	}
	public SortedWorkList(Comparator<Box> comparator) {
		super(new TreeSet<Box>(comparator), null);
		_sorter = comparator; // just to be on a safe side. if somewhere in the code somebody would refer to the _sorter 
	}
	...
	public int removeRejectedBoxes() {
		...
		Box leader = getBoxWithLowerBoundValue(); // SHOULD BE SO?
